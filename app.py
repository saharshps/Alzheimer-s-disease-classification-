import streamlit as st
import pickle
import pandas as pd

model = pickle.load(open('alzheimer_model.pkl', 'rb'))
median_values = pickle.load(open('median_values.pkl', 'rb'))


st.title("Alzheimer's Disease Prediction")
st.write("Enter patient details below")

mmse = st.number_input("MMSE Score (0-30)", 
                        min_value=0, max_value=30)

functional = st.number_input("Functional Assessment (0-10)", 
                              min_value=0, max_value=10)

adl = st.number_input("ADL Score (0-10)", 
                       min_value=0, max_value=10)

memory = st.selectbox("Memory Complaints", 
                       [0, 1], 
                       format_func=lambda x: "Yes" if x==1 else "No")

behavioral = st.selectbox("Behavioral Problems", 
                           [0, 1], 
                           format_func=lambda x: "Yes" if x==1 else "No")

if st.button("Predict"):


    input_data = median_values.copy()


    input_data['MMSE'] = mmse
    input_data['FunctionalAssessment'] = functional
    input_data['ADL'] = adl
    input_data['MemoryComplaints'] = memory
    input_data['BehavioralProblems'] = behavioral

    data = pd.DataFrame([input_data])

    result = model.predict(data)

    if result[0] == 1:
        st.error(" Alzheimer's Disease Detected")
        st.write("Please consult a doctor immediately")
    else:
        st.success(" No Alzheimer's Disease Detected")
        st.write("Patient appears healthy")
